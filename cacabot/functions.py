...
def Car_Price(car_price):
    ...
    jsonp_string = requests.get('http://www.carisyou.com/estimate/ajax/Compute.do?estimateType=1&useTerm=36&carGradeNbr=' + str(name)).text
    json_string = jsonp_string.replace('_(', '').replace(');', '')
...
def Car_PriceUrl(car_price):
    ...
    url = 'http://www.carisyou.com/estimate/5462?brandNbr=904&carGradeNbr=' + str(name)
